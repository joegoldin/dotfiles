{
  ...
  commonPackages = with pkgs; lib.flatten [
    act
...
in {
  home.packages = commonPackages ++ 
    (if pkgs.stdenv.isLinux && !(lib.hasAttr "wsl" config && config.wsl.enable) then nixosPackages else []) ++
    (if pkgs.stdenv.isDarwin then macosPackages else []) ++
    (if lib.hasAttr "wsl" config && config.wsl.enable then wslPackages else []);
