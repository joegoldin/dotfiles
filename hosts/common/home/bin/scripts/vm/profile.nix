  ...
  bash = ''
    ...
    case "$verb" in
      ls)
        ...
        for f in /var/lib/vm-specs/profiles/*.json; do
          [ -f "$f" ] || continue
          base=$(basename "$f" .json)
          desc=$(jq -r .description "$f" 2>/dev/null || echo "?")
          printf '  %-12s %s\n' "$base" "$desc"
        done
        if compgen -G "/var/lib/vm-specs/profiles/custom/*.json" >/dev/null; then
          echo ""
          bold "Custom profiles:"
          for f in /var/lib/vm-specs/profiles/custom/*.json; do
            base=$(basename "$f" .json)
            desc=$(jq -r .description "$f" 2>/dev/null || echo "?")
            printf '  %-12s %s\n' "$base" "$desc"
          done
      ...
      show)
        ...
        for f in "/var/lib/vm-specs/profiles/$name.json" "/var/lib/vm-specs/profiles/custom/$name.json"; do
          if [ -f "$f" ]; then
            jq . "$f"
            exit 0
          fi
      ...
      edit)
        ...
        custom="/var/lib/vm-specs/profiles/custom/$name.json"
