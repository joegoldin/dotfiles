# Pelican Panel and Wings configuration (standalone — siofra's own panel).
#
# Create admin user:
  ...
  den.aspects.siofra.nixos =
    ...
    in
    {
      ...
      services.pelican.panel = {
        ...
        group = "caddy";
        app = {
          url = "https://${domains.siofraPelicanDomain}";
          # Generate with: echo "base64:$(openssl rand -base64 32)"
          keyFile = config.age.secrets.siofra-pelican-app-key.path;
        };
        database.passwordFile = config.age.secrets.siofra-pelican-db-password.path;
        redis.passwordFile = config.age.secrets.siofra-pelican-redis-password.path;
      };
      ...
      # provides the public HTTPS endpoint (pelican.siofra.turnin.quest).
      services.caddy.enable = true;
      services.caddy.virtualHosts."http://${domains.siofraPelicanDomain}" = {
        extraConfig = ''
      ...
      services.pelican.wings = {
        ...
        # is created in siofra's panel.
        uuid = "REPLACE-WITH-SIOFRA-WINGS-NODE-UUID";
        remote = "https://${domains.siofraPelicanDomain}";
      ...
      # Generate secrets:
      #   App key: echo "base64:$(openssl rand -base64 32)" | agenix -e siofra-pelican-app-key.age
      #   DB password: openssl rand -base64 32 | agenix -e siofra-pelican-db-password.age
      #   Redis password: openssl rand -base64 32 | agenix -e siofra-pelican-redis-password.age
      #   Token ID & Token: get these from the panel after creating a node
      age.secrets = {
        siofra-pelican-app-key = {
          file = "${dotfiles-secrets}/siofra-pelican-app-key.age";
          mode = "0644";
        };
        siofra-pelican-db-password = {
          file = "${dotfiles-secrets}/siofra-pelican-db-password.age";
          mode = "0644";
        };
        siofra-pelican-redis-password = {
          file = "${dotfiles-secrets}/siofra-pelican-redis-password.age";
          mode = "0644";
        };
