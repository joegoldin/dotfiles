# Installer ISO for volcano-manor; an artifact build, not a machine, so it is a
# plain flake output rather than a den entity. It embeds the den-generated
# volcano-manor closure (self.nixosConfigurations.volcano-manor) plus all flake
...
in
{
  flake.nixosConfigurations.volcano-manor-installer =
    let
      targetSystem = self.nixosConfigurations.volcano-manor;
      ...
      modules = [
        ...
        (
          { pkgs, ... }:
          {
            ...
            # Auto-launch install-volcano-manor in Konsole
            environment.etc."xdg/autostart/install-volcano-manor.desktop".text = ''
              [Desktop Entry]
              Type=Application
              Name=Install Office PC
              Exec=kstart5 konsole -e install-volcano-manor
            ...
            environment.systemPackages = [
              ...
              (pkgs.writeShellScriptBin "install-volcano-manor" ''
                LOGFILE="/tmp/install-volcano-manor.log"
                ...
                on_error() {
                  ...
                  if findmnt /mnt &>/dev/null; then
                    sudo cp "$LOGFILE" /mnt/var/log/install-volcano-manor.log 2>/dev/null || true
                    echo "  Log also saved to /mnt/var/log/install-volcano-manor.log"
                  fi
                  echo ""
