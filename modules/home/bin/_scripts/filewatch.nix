{ pkgs }:
{
  ...
  params = [
    {
      name = "PATTERN";
      desc = "Glob pattern to watch (e.g. 'src/**/*.js')";
    }
  ];
  flags = [
    {
      ...
      short = "c";
      bool = true;
      desc = "Clear screen between runs";
    }
    {
      name = "restart";
      short = "r";
      bool = true;
      desc = "Restart long-running command on change (instead of waiting for exit)";
    }
  ];
  examples = [
    {
      cmd = "filewatch '*.js' -- make build";
      desc = "Rebuild on JS changes";
    }
    {
      cmd = "filewatch -r 'src/**/*.py' -- python app.py";
      desc = "Restart server on change";
    }
    {
      cmd = "filewatch -c '*.ts' -- npm test";
      desc = "Clear and run tests on change";
    }
    {
      cmd = "find src -name '*.js' | filewatch -- make build";
      desc = "Pipe file list directly";
    }
  ];
