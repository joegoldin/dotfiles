...
}:
{
  ...
  # When set to "prefer-wayland", DISPLAY is unset only if WAYLAND_DISPLAY is set, causing it to use Wayland only when Wayland is available.
  graphicsDriver ? "auto",
...
let
  ...
  launcher = writeShellScript "wine-launcher" ''
    ...
    source ${import ./filemap.nix { inherit writeScript rsync; }}
