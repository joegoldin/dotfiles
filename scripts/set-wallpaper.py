...
def find_random_image(folder_paths):
    ...
    if not image_files:
        print(f"Error: No supported image files found in any of the provided directories")
        sys.exit(1)
...
def get_monitor_info():
    """Get monitor information using KDE's DBus interface"""
    session_bus = dbus.SessionBus()
    
    try:
        # Use KScreen if available
        kscreen = session_bus.get_object('org.kde.KScreen', '/backend')
        kscreen_iface = dbus.Interface(kscreen, 'org.kde.kscreen.Backend')
        config = kscreen_iface.getConfig()
        
        # TODO: Process KScreen output (not implemented)
        print("KScreen method not implemented yet, falling back to xrandr")
        return get_monitor_info_xrandr()
    except:
        print("Exception, falling back to xrandr")
        # Fall back to xrandr
        return get_monitor_info_xrandr()

def get_monitor_info_xrandr():
        ...
        for line in output.split('\n'):
            ...
            if ' connected ' in line:
                ...
                if geometry:
                    ...
                    print(f"Found monitor: {name}, size: {width}x{height}, position: {x_pos}+{y_pos}")
                    monitors.append({
                        'name': name,
                        'width': width,
                        'height': height,
...
def set_kde_wallpaper(slices):
    ...
    script = """
// Reset all desktops to default settings first
var desktopArray = [];
...
"""
    ...
    file_urls = [f"file://{slice['path']}" for slice in slices]
    file_list_str = ', '.join('"' + item + '"' for item in file_urls)
    
    # Connect to D-Bus session bus
    session_bus = dbus.SessionBus()
    
    # Get the Plasma Shell interface
    plasma = session_bus.get_object("org.kde.plasmashell", "/PlasmaShell")
    interface = dbus.Interface(plasma, "org.kde.PlasmaShell")
    
    # Execute the script with the image paths
    interface.evaluateScript(script.format(file_list_str))
